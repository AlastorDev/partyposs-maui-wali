...
export const SHOP = {berries:{name:'Campfire Story card',price:25,amount:3,detail:'Three stories that build trust with critters.',icon:'<span class="camp-item story"></span>'},ultras:{name:'Golden Woggle',price:60,amount:2,detail:'Two special woggles for stronger invitations.',icon:'<span class="camp-item golden"></span>'},potions:{name:'Trail tonic',price:35,amount:2,detail:'Two tonics. Restore 60% health in battle.',icon:'✚'}};
...
export function startBattle(kind,zoneId,enemy,p=profile){const index=ZONES.findIndex(z=>z.id===zoneId);if(index<0||(kind!=='lodge'&&!zoneOpen(index,p))||!p.team.some(id=>owned(id,p)?.hp>0))return null;const queue=kind==='trial'?ZONES[index].foes.map(([id,level])=>makeUnit(id,level)):kind==='lodge'&&Array.isArray(enemy)?enemy.map(u=>({...u})):[{...enemy}];if(!queue.length)return null;if(queue.some(u=>!SPECIES[u.species]))return null;return{kind,zoneId,queue,index:0,enemy:queue[0],active:p.team.find(id=>owned(id,p)?.hp>0),energy:0,round:0,status:'active',log:kind==='trial'?`${ZONES[index].trial} begins!`:kind==='lodge'?'The Lodge guardians welcome your patrol!':`A wild ${SPECIES[enemy.species].name} steps forward.`,resultPaid:false};}
export function battleTurn(battle,action,p=profile,random=Math.random){
  ...
  battle.round++;
  if(battle.enemy.hp<=0){battle.index++;if(battle.index>=battle.queue.length){battle.status='won';log.push(battle.kind==='trial'?'Trial complete!':battle.kind==='lodge'?'The Lodge salutes your patrol!':'The wild critter is ready to join you.');}else{battle.enemy=battle.queue[battle.index];log.push(`${SPECIES[battle.enemy.species].name} steps forward.`);}battle.log=log.join(' ');return{ok:true,move,dealt};}
